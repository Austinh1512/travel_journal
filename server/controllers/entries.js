...
module.exports.getEntries = async (req, res, next) => {
    ...
    if (!mongoose.isObjectIdOrHexString(userID)) return res.status(500).json({ "error": "Not a valid user ID" });
...
module.exports.editEntry = async (req, res) => {
    ...
    if (!mongoose.isObjectIdOrHexString(id)) return res.status(500).json({ "error": "Not a valid entry ID" });

    const updated_entry = await JournalEntry.findOneAndUpdate({ _id: id }, req.body, { runValidators: true, new: true });
...
module.exports.deleteEntry = async (req, res) => {
    ...
    return res.status(200).json({"type": "success", "message": "Successfully deleted entry!"});
