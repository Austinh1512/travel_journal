...
router.route("/")
    .post(passport.authenticate("jwt", { session: false }), upload.any(), validateEntrySchema, handleAsync(addEntry))

router.route("/:userID").get(handleAsync(getEntries));
    
router.route("/:id")
    .put(passport.authenticate("jwt", { session: false }), validateEntrySchema, handleAsync(editEntry))
